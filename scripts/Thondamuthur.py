...
# ==============================================================================
# Hex color profiles mapped to your 5 core parties and 1 aggregated independent key
party_config = {
...
# Standardize column spacing and strip double/triple whitespaces to prevent key errors
df.columns = df.columns.str.replace(r'\s+', ' ', regex=True).str.strip()

# Dynamic Matching Check: Only isolate configured parties that exist inside this CSV
...
# Direct mapping variables configured precisely to your exact new column text strings
station_col = 'Serial No. Of Polling Station'
locality_col = 'Polling Station Name'
...
# CRITICAL TYPE FIX: Prevents object vs int64 future dataset merge crashes
df[station_col] = pd.to_numeric(df[station_col], errors='coerce').fillna(0).astype('int64')

# Isolate all 16 unique independent column variations dynamically
non_independent_meta_cols = base_party_columns + [
    station_col, locality_col, area_col, voter_type_col,
    'Total of Valid Votes', 'No. Of Rejected Votes', 'NOTA', 'Total', 'No. Of Tendered Votes', 'Sl. No'
]
independent_cols = [c for c in df.columns if 'independent' in c.lower() and c not in non_independent_meta_cols]

# Create a clean single Independent aggregator column summing up all 16 lanes safely
if independent_cols:
...
# Ensure chronological ordering holds before visualizations
df = df.sort_values(by=station_col, ascending=True).reset_index(drop=True)

# ==============================================================================
# CONSOLE SUMMARY EXPLOIT
# ==============================================================================
print("\n====================================")
...
for sheet in wb.worksheets:
    ...
    for col in sheet.columns:
        ...
        # PERFECT FIX: Query attribute metrics from the first cell instance inside the tuple array
...
print(f"\n🚀 SUCCESS! Steps 4 to 7 executed with perfect tuple alignment: '{excel_filename}'")
