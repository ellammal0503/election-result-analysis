...
# ==============================================================================
# Refitted to match the exact 4 primary contestants present in your new schema
party_config = {
...
# Load your file here (Make sure to replace with your actual raw filename)
df = pd.read_csv("All.csv")

# CRITICAL FIX: Merge wide multiple whitespaces down to a unified single blank character
...
# Isolate the 5 explicit independent candidate fields present in this dataset
independent_cols = ['Independent', 'Independent.1', 'Independent.2', 'Independent.3', 'Independent.4']
existing_ind_cols = [c for c in independent_cols if c in df.columns]
if existing_ind_cols:
    df[existing_ind_cols] = df[existing_ind_cols].fillna(0)
...
if existing_ind_cols:
    df['Total_Independent_Votes'] = df[existing_ind_cols].sum(axis=1)
...
for sheet in wb.worksheets:
    # 1. FIX: Explicitly target and format individual cell objects inside Row 1 only
    for cell in sheet[1]:
        cell.font = Font(bold=True, color="FFFFFF")
        cell.fill = PatternFill(start_color="1F4E78", end_color="1F4E78", fill_type="solid")
    
    # 2. FIX: Unpack layout mapping widths by referencing the cell's column index safely
    for col in sheet.columns:
        max_len = 0
        actual_column_index = col[0].column # Read index property from the actual cell object
