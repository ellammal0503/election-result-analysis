...
# ==============================================================================
# Adjusted to match the exact single-space text keys after stripping newlines (\n)
party_config = {
...
# CRITICAL FIX: Strip raw line breaks (\n) and merge wide spaces across columns
...
# Updated variable layout anchors to mirror your exact column strings
station_col = 'Polling Station No.'
locality_col = 'Location and Name of the Building in which Polling station is located'
area_col = 'Polling Area'

df[station_col] = pd.to_numeric(df[station_col], errors='coerce').fillna(0).astype(int)

# Group structural columns to isolate the 2 Independents ('Independent' and 'Independent.1')
non_independent_cols = party_columns + [
    'SL. NO.', station_col, locality_col, area_col, 'Total of valid votes', 
    'No. of rejected Votes', 'NOTA', 'Total', 'No. of tendered votes', 'S No', 
...
for sheet in wb.worksheets:
    # 1. FIX: Explicitly target and format only the individual cell items inside Row 1
    for cell in sheet[1]:
