...
# ==============================================================================
# Adjust this filename string to point directly to your original source file
input_file = "All.csv" 
...
# ==============================================================================
# STEP 1: DEFINE SYSTEM CONFIGURATION (Lowercased to match your exact schema)
# ==============================================================================
party_config = {
...
# ==============================================================================
# TYPE CASTING & VALUES VALIDATION
# ==============================================================================
station_col = [c for c in df.columns if 'serial no.' in str(c) or 'station' in str(c)][0]
locality_col = 'location_clean'  # Utilizing clean split column for aggregation
...
vote_cols_to_cast = party_columns + ['total of valid votes']
...
# ==============================================================================
# STEP 3: ADVANCED STRATEGIC METRICS (INDIVIDUAL BOOTH-LEVEL SERIAL NUMBERS)
# ==============================================================================
print("Extracting strategic election metrics at individual polling station levels...")

# Safely search for the lowercased total votes column name dynamically
total_votes_col = [c for c in df.columns if 'total of valid votes' in str(c) or 'total' in str(c)][0]

# Safeguard column name string extractions from our initialization steps
s_col = station_col if isinstance(station_col, list) else station_col
t_col = total_votes_col if isinstance(total_votes_col, list) else total_votes_col

# 1. Update Margin Percentage
df['Margin_Percentage'] = np.where(
    df[t_col] > 0, 
    (df['Margin_Of_Victory'] / df[t_col]) * 100, 
    0.0
)

# 2. Critical Swings Sheet (Booth Level)
critical_swings = df[df['Margin_Percentage'] <= 5.0][
    [s_col, locality_col, area_col if isinstance(area_col, list) else area_col, 
     'Winner_Party', 'Runner_Up_Party', 'Margin_Of_Victory', 'Margin_Percentage']
].sort_values(by=s_col, ascending=True).copy()

critical_swings = critical_swings.rename(columns={s_col: 'Polling_Station_No'})


# 3. Dominant Booth Sheet (Converted to Individual Booth Level)
locality_dominance_summary = df[
    [s_col, locality_col, 'Winner_Party', 'Winner_Votes', 'Margin_Of_Victory']
].sort_values(by=s_col, ascending=True).copy()

locality_dominance_summary = locality_dominance_summary.rename(columns={
    s_col: 'Polling_Station_No',
    'Winner_Party': 'Dominant_Party_In_Booth',
    'Winner_Votes': 'Dominant_Party_Votes'
})


# 4. Independent Split Sheet (Converted to Individual Booth Level)
...
independent_vote_splitting = df[
    [s_col, locality_col, t_col, 'Total_Independent_Votes', 'Winner_Party', 'Margin_Of_Victory']
].copy()

independent_vote_splitting['Independent_Vote_Share_%'] = np.where(
    independent_vote_splitting[t_col] > 0,
    (independent_vote_splitting['Total_Independent_Votes'] / independent_vote_splitting[t_col]) * 100,
    0.0
)

# Identify if independents pooled more votes than the margin of victory in that specific station
...
independent_vote_splitting = independent_vote_splitting.rename(columns={s_col: 'Polling_Station_No'})


# Final sorting reset for the master dataframe
df = df.sort_values(by=s_col, ascending=True).reset_index(drop=True)
print("Strategic layers updated! All analysis tabs now reference explicit polling station serial numbers.")
# ==============================================================================
# STEP 4: GENERATE MACRO CONSTITUENCY DASHBOARD CHART
# ==============================================================================
print("\nCompiling constituency macro-level summary dashboard...")

# Dynamically locate the lowercase total votes column name safely
total_votes_col = [c for c in df.columns if 'total of valid votes' in str(c) or 'total' in str(c)][0]
...
plt.close()
# ==============================================================================
# STEP 5: INDIVIDUAL BOOTH VISUALS (WITH FULL BUILDING AND AREA TEXT INSIDE IMAGE)
# ==============================================================================
print("Generating dynamically sorted booth images with full location metadata...")

# Ensure column references extract clean string handles even if formatted as single-element lists
s_col = station_col if isinstance(station_col, list) else station_col
t_col = total_votes_col if isinstance(total_votes_col, list) else total_votes_col

# Explicitly identify the full lowercased text descriptive columns from your schema
building_raw_col = 'building in which it will be located'
polling_areas_col = 'polling areas'

for index, row in df.iterrows():
    station_id = str(int(row[s_col]))
    total_votes = row[t_col]
    
    # Extract the full descriptive texts requested
    building_name = str(row[building_raw_col]).strip() if building_raw_col in df.columns else "N/A"
    polling_areas_text = str(row[polling_areas_col]).strip() if polling_areas_col in df.columns else "N/A"
    ...
    # Expanded figure height slightly to cleanly balance the extra multi-line text blocks
    ...
    # Wrap both string inputs independently to ensure beautiful horizontal grid alignments
    wrapped_building = "\n".join(textwrap.wrap(f"Building: {building_name}", width=95))
    wrapped_areas = "\n".join(textwrap.wrap(f"Polling Areas: {polling_areas_text}", width=95))
    
    # Combined multi-line text stamp for the plot title layout
    ...
    # Sanitize building text strings to safely build filesystem-compliant output file names
...
print(f"All booth graphics generated with nested location metadata arrays inside folder: '{output_dir}/'")
...
# Safely open to format with openpyxl outside the pandas writer context block
...
# ==============================================================================
# STEP 7: AUTO-FIT COLUMN WIDTHS ACROSS ALL WORKPASS TABS (FIXED FOR OPENPYXL TUPLES)
# ==============================================================================
print("Optimizing Excel grid padding layout widths...")
for sheet in wb.worksheets:
    for col in sheet.columns:
        max_len = 0
        # Access the first cell object inside the column tuple to safely read the column number
        ...
        # Apply padding safely without cutting off long header descriptions
