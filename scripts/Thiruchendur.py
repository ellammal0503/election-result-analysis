...
# CRITICAL FIX: Standardize spacing and replace hidden newline escape tokens (\n) across headers safely
...
# Explicitly tie structural column parameters matching your precise new index layout
...
voter_type_col = 'Polling Station Type'
...
# Force booth serial numbers to int64 to completely eliminate type mismatch failures
df[station_col] = pd.to_numeric(df[station_col], errors='coerce').fillna(0).astype('int64')

# Isolate all 5 unique independent column variations dynamically
non_independent_meta_cols = base_party_columns + [
    station_col, locality_col, area_col, voter_type_col,
...
# Create a clean single Independent aggregator column summing up all 5 lanes safely
if independent_cols:
...
# FIXED: Focus check directly on column shape dimensions to eliminate direct tuple evaluations
...
# ==============================================================================
# STEP 7: AUTO-FIT COLUMN WIDTHS & STYLING (ABSOLUTE FIX FOR OPENPYXL TUPLES)
# ==============================================================================
print("Optimizing Excel grid padding layout widths and styles...")

for sheet in wb.worksheets:
    # 1. Format headers safely on Row 1
    for row in sheet.iter_rows(min_row=1, max_row=1):
        for cell in row:
            cell.font = Font(bold=True, color="FFFFFF")
            cell.fill = PatternFill(start_color="1F4E78", end_color="1F4E78", fill_type="solid")
    
    # 2. Iterate through columns as raw immutable tuples safely
    for col in sheet.columns:
        ...
        # CRITICAL RE-INDEX: Extract the column property from the first cell of the tuple block
...
print(f"\n🚀 SUCCESS! Reporting workbook environment finalized: '{excel_filename}'")
