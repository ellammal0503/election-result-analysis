...
# ==============================================================================
# STEP 1: DEFINE SYSTEM CONFIGURATION (EXACT 15 COLUMNS AND COLORS)
# ==============================================================================
party_config = {
    ...
    'Communist Party of India (Marxist-Leninist) (Liberation)': {'label': 'CPI_ML', 'color': '#7B1E1E'},
    'Makkal Nalvaazhvuk Katchi': {'label': 'MNK', 'color': '#7B1FA2'},
...
# Handle dynamic name parameters to block look-up tracking errors
station_col = [c for c in df.columns if 'Station' in str(c)][0]
locality_col = [c for c in df.columns if 'Locality' in str(c)][0]
...
for index, row in df.iterrows():
    ...
    # Window scaling allows clear presentation layout for all 15 elements
    ...
    for bar, pct, count in zip(bars, station_data['Percentage'], station_data['Votes']):
        width = bar.get_width()
        # Clean string label format text override
...
for sheet in wb.worksheets:
    for col in sheet.columns:
        max_len = 0
        # Fixes tuple tracking indexing bug by extracting index property from coordinates cell directly
