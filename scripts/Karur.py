...
# ==============================================================================
# STEP 1: DEFINE SYSTEM CONFIGURATION (EXACT MATCH FOR NEW SCHEMA)
# ==============================================================================
# Branded hex color codes configured for your 9 core parties and 1 aggregated independent key
party_config = {
    ...
    'Ganasangam Party of India': {'label': 'GSPI', 'color': '#6A1B9A'},
...
# Standardize column spacing and strip hidden trailing whitespaces to prevent key errors
...
locality_col = 'Locality of Polling Station'
building_col = 'Building in which it will be Located'
area_col = 'Polling Area'
voter_type_col = 'Whether for all Voters of men only or women only'
...
# Isolate all 71 unique independent columns dynamically
non_independent_meta_cols = base_party_columns + [
    'Serial No.', station_col, locality_col, building_col, area_col, voter_type_col,
...
# Create a clean single Independent aggregator column summing up all 71 lanes safely
if independent_cols:
...
critical_swings = df[df['Margin_Percentage'] <= 5.0][
    [station_col, building_col, area_col, 'Winner_Party', 'Runner_Up_Party', 'Margin_Of_Victory', 'Margin_Percentage']
...
# ==============================================================================
# SUMMARY EXPLOIT: PRINT DETAILED MARGIN REPORT OVER CONSOLE
# ==============================================================================
print("\n====================================")
...
for cluster_num in sorted(df['Cluster_ID'].unique()):
    ...
    target_cols = [station_col, locality_col, building_col, area_col, 'Winner_Party', 'Runner_Up_Party', 'Margin_Percentage']
...
# Re-load workbook to safely inject assets via openpyxl drawing tools
...
# ==============================================================================
# STEP 7: AUTO-FIT COLUMN WIDTHS & STYLING (PERFECT FIX FOR OPENPYXL TUPLES)
# ==============================================================================
...
for sheet in wb.worksheets:
    ...
    # 2. SAFELY UNPACK TUPLE: Pull structural index parameters from the first cell instance
    for col in sheet.columns:
        ...
        # PERFECT FIX: Query index mapping attributes safely using index boundary position [0]
...
print(f"\n🚀 SUCCESS! All systems operational. Reporting workbook environment finalized: '{excel_filename}'")
