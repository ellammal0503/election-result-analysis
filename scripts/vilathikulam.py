...
# ==============================================================================
# Customized brand colors mapping exactly to your 8 core parties and 1 aggregated independent key
party_config = {
...
valid_votes_col = 'Total of Valid Votes'
...
non_independent_meta_cols = base_party_columns + [
    station_col, valid_votes_col, locality_col, area_col, voter_type_col,
...
df['Margin_Percentage'] = np.where(
    df[valid_votes_col] > 0, 
    (df['Margin_Of_Victory'] / df[valid_votes_col]) * 100, 
    0
)
...
independent_vote_splitting = df.groupby(locality_col).agg(
    Total_Valid_Votes_In_Locality=(valid_votes_col, 'sum'),
...
for col in all_competing_columns:
    ...
    df[col_name] = np.where(df[valid_votes_col] > 0, (df[col] / df[valid_votes_col]) * 100, 0)
...
total_valid_constituency_votes = df[valid_votes_col].sum()
...
for index, row in df.iterrows():
    ...
    total_votes = row[valid_votes_col]
...
for sheet in wb.worksheets:
    ...
    # Iterate through columns as raw cell tuples
    for col in sheet.columns:
        ...
        # PERMANENT TUPLE FIX: Pull the column index from the first cell element in the tuple
