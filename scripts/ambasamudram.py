...
# ==============================================================================
# STEP 1: DEFINE SYSTEM CONFIGURATION (EXACT COLUMNS FROM DATASET AND COLORS)
# ==============================================================================
# Matched strictly against your current dataset columns
party_config = {
    ...
    'Indian National Congress': {'label': 'INC', 'color': '#1565C0'},
...
# Dynamic name parameters matched to your exact dataset column structure
...
# Identify other candidates dynamically if any are present (excluding main parties, totals, and metadata)
...
for bar, pct, votes in zip(macro_bars, summary_df['Share_Percentage'], summary_df['Total_Votes']):
    width = bar.get_width()
    ax.text(width + (summary_df['Share_Percentage'].max() * 0.02), bar.get_y() + bar.get_height()/2, f"{int(votes):,} Votes ({pct:.2f}%)", 
...
for index, row in df.iterrows():
    ...
    for bar, pct, count in zip(bars, station_data['Percentage'], station_data['Votes']):
        ...
        ax.text(width + (station_data['Percentage'].max() * 0.02), bar.get_y() + bar.get_height()/2, label_text, 
    ...
    ax.set_xlim(0, min(100, station_data['Percentage'].max() * 1.3))  
...
# Embed core dashboard visual block into its dedicated tab safely
...
for sheet in wb.worksheets:
    # 1. Format headers to stand out cleanly
    for cell in sheet[1]:
